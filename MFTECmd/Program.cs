...
public class Program
{
    ...
    private static readonly string Header =
        ...
        "\r\n\r\nCore Author: Eric Zimmerman (saericzimmerman@gmail.com)" +
        "\r\nReworked By: Christiaan Masucci" +
    ...
    private static async Task Main(string[] args)
    {
        ...
        _rootCommand = new RootCommand
        {
            ...
            new Option<bool>(
                "-r",
                () => false,
                "Run the resident filter that only shows resident files and produces the resident data associated with them"),

            new Option<bool>(
                "-e",
                () => false,
                "Make the csv file compatible for excel files, this will split the file to multiple if it is too large"),
    ...
    private static void DoWork(string f, bool r, bool e, string m, string json, string jsonf, string csv, string csvf, string body, string bodyf, string bdl, bool blf, string dd, string @do, string de, bool dr, bool fls, string ds, string dt, bool sn, bool fl, bool at, bool rs, bool vss, bool dedupe, bool debug, bool trace)
    {
        ...
        switch (ft)
        {
            ...
            case FileType.Mft:
                ...
                ProcessMft(f, r, e, vss, dedupe, body, bdl, bodyf, blf, csv, csvf, json, jsonf, fl, dt, dd, @do, fls, sn, at, de,rs,drDir);
                break;
            ...
            case FileType.UsnJournal:
                ...
                if (m.IsNullOrEmpty() == false)
                {
                    ...
                    ProcessMft(m, r, e, vss, dedupe, body, bdl, bodyf, blf, csv, csvf, json, jsonf, fl, dt, dd, @do, fls, sn, at, de,rs,drDir2);
                }
        ...
        foreach (var mftFile in mftFiles)
        {
                ...
                json.IsNullOrEmpty() == false)
            {
                if (csv.IsNullOrEmpty() == false)
                {
                    ...
                    outNameExtra = "F2" + outName;
                    outFile = Path.Combine(csv, outName);
                    outFileExtra = Path.Combine(csv, outNameExtra);
                    ...
                    else         //files that list all data will need to be split into two csv files for excel to list all the data
                    {
                        try
                        {
                            swCsv = new StreamWriter(outFile, false, Encoding.UTF8, 4096 * 4);
                            swCsvExtra = new StreamWriter(outFileExtra, false, Encoding.UTF8, 4096 * 4);

                            _csvWriter = new CsvWriter(swCsv, CultureInfo.InvariantCulture);
                            _csvWriterExtra = new CsvWriter(swCsvExtra, CultureInfo.InvariantCulture);

                            var foo = _csvWriter.Context.AutoMap<MFTRecordOut>();
                            var fooE = _csvWriterExtra.Context.AutoMap<MFTRecordOut>();
                            var temp = new MFTRecordOut();
                            ...
                            fooE.Map(t => t.EntryNumber).Index(0);
                            fooE.Map(t => t.SequenceNumber).Index(1);
                            fooE.Map(t => t.InUse).Index(2);
                            fooE.Map(t => t.ParentEntryNumber).Index(3);
                            fooE.Map(t => t.ParentSequenceNumber).Index(4);
                            fooE.Map(t => t.ParentPath).Index(5);
                            fooE.Map(t => t.FileName).Index(6);
                            ...
                            _csvWriterExtra.Context.RegisterClassMap(fooE);

                            _csvWriter.WriteHeader<MFTRecordOut>();
                            _csvWriter.NextRecord();

                            _csvWriterExtra.WriteHeader<MFTRecordOut>();
    ...
    private static void ProcessRecords(Dictionary<string, FileRecord> records, bool includeShort, bool alltimestamp, string bdl,string drDumpDir, bool res, bool exc)
    {
        ...
        foreach (var fr in records)
        {
                         ...
                         t.AttributeType == AttributeType.FileName).OrderBy(t => ((FileName)t).FileInfo.NameType))
            {
                ...
                if(!res)
                {
                    if (recordNum < 1000000)      //if there are less than 800000 entrys then write to the second file
                    {
                    ...
                    {
                        _csvWriterExtra?.WriteRecord(mftr);
